...
export const renderNotification = (app, container) => {
    if (app.state.notification.visible) {
        const notifHtml = `
            <div class="fixed top-6 left-1/2 -translate-x-1/2 z-[200] animate-fade-in">
                <div class="px-6 py-3 rounded-2xl shadow-xl border ${app.state.notification.type === 'error' ? 'bg-red-50 border-red-100 text-red-700' : 'bg-emerald-50 border-emerald-100 text-emerald-700'} font-medium flex items-center gap-3">
                    ${app.state.notification.type === 'error' ? '⚠️' : '✅'} ${app.state.notification.message}
                </div>
            </div>`;
        ...
        const modalHtml = `
            <div class="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
                <div class="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-xl">
                    ...
                    <p class="text-stone-50 mb-6 border-b pb-4 text-stone-600">
